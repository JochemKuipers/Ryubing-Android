...
static u128 VectorGetElement(u128 value, u64 bitsize) {
    switch (bitsize) {
    case 8:
        return value & u128((1ULL << 8) - 1);
    case 16:
        return value & u128((1ULL << 16) - 1);
    case 32:
        return value & u128((1ULL << 32) - 1);
    case 64:
        return value & u128(~0ULL);
    default:
        return value;
    }
}
...
bool InterpreterVisitor::Ordered(size_t size, bool L, bool o0, Reg Rn, Reg Rt) {
    ...
    case MemOp::Store: {
        ...
        if (!m_memory.WriteBlock(address, &value, dbytes))
            return false;
        std::atomic_thread_fence(std::memory_order_seq_cst);
        break;
    }
    case MemOp::Load: {
        u64 value = 0;
        if (!m_memory.ReadBlock(address, &value, dbytes))
            return false;
...
bool InterpreterVisitor::LDR_lit_gen(bool opc_0, Imm<19> imm19, Reg Rt) {
    ...
    u64 data = 0;
    if (!m_memory.ReadBlock(address, &data, size))
        return false;
    this->SetReg(Rt, data);
    return true;
}
...
bool InterpreterVisitor::STP_LDP_gen(Imm<2> opc, bool not_postindex, bool wback, Imm<1> L,
    ...
    case MemOp::Store: {
        ...
        if (!m_memory.WriteBlock(address, &data1, dbytes))
            return false;
        if (!m_memory.WriteBlock(address + dbytes, &data2, dbytes))
            return false;
        break;
    }
    case MemOp::Load: {
        u64 data1 = 0, data2 = 0;
        if (!m_memory.ReadBlock(address, &data1, dbytes))
            return false;
        if (!m_memory.ReadBlock(address + dbytes, &data2, dbytes))
            return false;
        if (signed_) {
...
bool InterpreterVisitor::RegisterImmediate(bool wback, bool postindex, size_t scale, u64 offset,
    ...
    case MemOp::Store: {
        ...
        if (!m_memory.WriteBlock(address, &data, datasize / 8))
            return false;
        break;
    }
    case MemOp::Load: {
        u64 data = 0;
        if (!m_memory.ReadBlock(address, &data, datasize / 8))
            return false;
        if (signed_) {
...
std::optional<u64> MatchAndExecuteOneInstruction(HostMappedMemory& memory, mcontext_t* context,
                                                 fpsimd_context* fpsimd_context) {
    std::span<u64, 31> regs(reinterpret_cast<u64*>(context->regs), 31);
    std::span<u128, 32> vregs(reinterpret_cast<u128*>(fpsimd_context->vregs), 32);
    // Android mcontext uses __u64 (unsigned long long); our u64 is uint64_t
    // (unsigned long on LP64). Same width — bind via reinterpret_cast.
    u64& sp = *reinterpret_cast<u64*>(&context->sp);
    const u64 pc = static_cast<u64>(context->pc);
    ...
    bool was_executed = decoder.value_or(false);
