 ...
 *
 * Vendored for Ryubing-Android NCE: fmt::formatter specializations and
 * <fmt/format.h> removed; ToReg/ToVec helpers added.
 */
...
inline Reg ToReg(size_t number) {
    ASSERT(number <= 31);
    return static_cast<Reg>(number);
}

inline Vec ToVec(size_t number) {
    ASSERT(number <= 31);
    return static_cast<Vec>(number);
}
