...
//
// ARM64 instruction decode for the NCE patcher.
//
// Ported from eden's src/core/arm/nce/instructions.h (originally from the
// Skyline emulator, MPL-2.0). Only the bitfield helper is replaced with a
// minimal local implementation to avoid a dependency on eden's common lib.
...
// Minimal bitfield extractor (subset of eden's Common::BitField).
template <unsigned Position, unsigned Bits, typename T>
struct BitField {
    static constexpr T Mask = Bits == sizeof(T) * 8 ? ~T(0) : (T(1) << Bits) - 1;

    static constexpr T ExtractValue(T value) {
        return (value >> Position) & Mask;
    }

    static constexpr T FormatValue(T value) {
...
union SVC {
    ...
    constexpr bool Verify() const {
        return (GetSig0() == 0x1 && GetSig1() == 0x6A0);
    }

    constexpr u32 GetSig0() const { return decltype(sig0)::ExtractValue(raw); }
    constexpr u32 GetValue() const { return decltype(value)::ExtractValue(raw); }
    constexpr u32 GetSig1() const { return decltype(sig1)::ExtractValue(raw); }
...
union MRS {
    ...
    constexpr bool Verify() const { return (GetSig() == 0xD53); }

    constexpr u32 GetRt() const { return decltype(rt)::ExtractValue(raw); }
    constexpr u32 GetSystemReg() const { return decltype(system_reg)::ExtractValue(raw); }
    constexpr u32 GetSig() const { return decltype(sig)::ExtractValue(raw); }
...
union MSR {
    ...
    constexpr bool Verify() const { return GetSig() == 0xD51; }
...
union Exclusive {
    ...
    constexpr bool Verify() const { return GetSig() == 0x10; }
...
static_assert(sizeof(Exclusive) == sizeof(u32));
